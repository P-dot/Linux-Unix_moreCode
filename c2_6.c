...
#include <stdlib.h>
#include <string.h>
/*This example program assumes dlist is represented by a NODE structure
  which os initialized with both pointers to the NODE structure itself */
...
int main()
{
    ...
    while(1){
       ...
       scanf("%d", &key);
       if (key < 0) exit (0);  // exit if key is negative
       p = search(&dlist, key);
       if (p==0){
